...
INSERT INTO todos (body, username) VALUES (?, ?) RETURNING id, body, username
`
...
func (q *Queries) InsertTodo(ctx context.Context, arg InsertTodoParams) (Todo, error) {
	...
	var i Todo
	err := row.Scan(&i.ID, &i.Body, &i.Username)
	return i, err
}
...
SELECT id, author, message, room_id, created_at FROM messages WHERE room_id = ? ORDER BY created_at DESC LIMIT ?
`
...
SELECT id, author, message, room_id, created_at FROM messages WHERE author = ? ORDER BY created_at DESC LIMIT ?
`
...
SELECT id, body, username FROM todos WHERE username = ?
`
...
func (q *Queries) SelectTodosByUsername(ctx context.Context, username string) ([]Todo, error) {
	...
	for rows.Next() {
		var i Todo
		if err := rows.Scan(&i.ID, &i.Body, &i.Username); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
//
//	SELECT user_id FROM users_tokens WHERE token_id = ? LIMIT 1
func (q *Queries) SelectUserIdFromToken(ctx context.Context, tokenID string) (string, error) {
...
UPDATE messages SET message = ? WHERE id = ? RETURNING id, author, message, room_id, created_at
`
...
UPDATE todos SET body = ? WHERE id = ? RETURNING id, body, username
`
